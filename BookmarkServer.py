...
class Shortener(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("You need to fill out both fields in the form".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("We tested, but found that {} does not exist or is not reachable".format(longuri).encode())
